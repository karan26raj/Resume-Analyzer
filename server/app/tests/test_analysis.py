...
def _create_user_and_headers(
    ...
    email = f"analysis-user-{uuid.uuid4()}@example.com"
...
def test_match_creates_and_stores_analysis(
    client,
    db_session,
    monkeypatch
):

    user, headers = _create_user_and_headers(
        client,
        db_session
    )

    resume, job = _create_resume_and_job(
        db_session,
        user
    )
...
def test_match_rejects_resume_not_owned(
    client,
    db_session,
    monkeypatch
):

    owner, _ = _create_user_and_headers(
        client,
        db_session
    )
    ...
    _, other_headers = _create_user_and_headers(
        client,
        db_session
    )
...
def test_match_rejects_empty_resume(
    client,
    db_session,
    monkeypatch
):
...
def test_match_returns_safe_error(
    client,
    db_session,
    monkeypatch
):
